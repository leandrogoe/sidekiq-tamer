RSpec.describe(SidekiqResourceGuard::Mongo::MongoServer) do
  ...
  describe 'unauthorized' do
    before(:each) do
      SidekiqResourceGuard::Mongo::MongoConfiguration.setup(user: 'USER', password: 'PASS', ticket_threshold: 80)
    end

    it 'fails to query tickets if it does not have the appropriate role' do
      server = SidekiqResourceGuard::Mongo::MongoServer.new(host, port)
  ...
  describe 'with proper access' do
    before(:each) do
      SidekiqResourceGuard::Mongo::MongoConfiguration.setup(user: 'clusterMonitor', password: 'PASS', ticket_threshold: 80)
    end
